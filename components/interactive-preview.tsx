...
export default function InteractivePreview({ imageData, pixelatedSrc, isProcessing }: InteractivePreviewProps) {
  ...
  return (
    <div className="flex-1 flex flex-col gap-2 min-h-0">
      <div
        ref={containerRef}
        className="flex-1 border border-border rounded-sm bg-secondary overflow-hidden cursor-move relative touch-none"
      ...
      {/* Control Bar */}
      <div className="flex items-center gap-2 bg-card border border-border rounded-sm p-2">
        {/* Zoom Controls */}
        <button
          onClick={() => setZoom((z) => Math.max(10, z - 10))}
          title="Zoom Out (−)"
          className="p-1.5 bg-secondary border border-border rounded-xs hover:bg-muted transition-colors text-foreground text-xs font-bold"
        >
          −
        </button>
        ...
        <div className="w-12 text-center text-xs font-medium text-foreground bg-secondary border border-border rounded-xs px-2 py-1">
          {zoom}%
        </div>
        ...
        <button
          ...
          className="px-2 py-1 text-xs font-medium bg-secondary border border-border rounded-xs hover:bg-muted transition-colors text-foreground"
        >
          Fit
        </button>
